...
For a quantitative analysis the significance quotient has to be calculated from the log-likelehood function $f(x)=-2\Delta log(L)$.
The significance can be calculate using $Z(x)=\sqrt{f(x)}$ resulting in $S = \frac{Z_{GM}(0)}{Z_{EFT}(0)}$.
...
The value of $f(0)$ can be read off from the Log-likelihood-funktion in \ref{fig:EFT_GM_Asimov_comparision}. The scale is dependent on
...
Using this the error becomes small compared to the statistical errors. All EFT coefficients in \ref{fig:significans_plots} with $S>0.8$ are able to reproduce the GM Model with a statistical significant greater than $80\%$.
...
The cross-section can't be scaled higher without breaking the fitting-tool as the values get unreasonably high. In the invariant mass plot for 0.5 times the cross-section, the cross-section becomes to low to be picked up by the fitting tool but in the transverse mass plot
...
The minimal range should leave space for the fit to find the best fit value. \textcolor{blue}{T0 doesn't show this behaviour and combined with the in \ref{fig:corss-section-comparission} shown ability to accurately describe low cross-section resonances may hint to a smaller impact of the Quadratic Term from T0.}
...
\begin{table}[h]
    ...
    \caption{Invariant mass lower and upper $95\%$ confidence level limits for a fit with linear and quadratic operators in WZ with all other aQCD parameters set to zero}
